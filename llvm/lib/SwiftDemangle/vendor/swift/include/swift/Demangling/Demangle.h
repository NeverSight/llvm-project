...
#include "SwiftDemangleBudget.h" // LLVM integration: legacy formatting budgets.
...
public:
  ...
  DemanglerPrinter &operator<<(llvm::StringRef Value) & {
    llvm::swift_demangle_detail::appendString(Stream, std::string_view(Value));
    return *this;
  }
  
  DemanglerPrinter &operator<<(char c) & {
    llvm::swift_demangle_detail::appendChar(Stream, c);
    return *this;
  }
