...
#include "SwiftDemangleBudget.h" // LLVM integration: parser resource limits.
...
public:
  ...
  template<typename T> T *Allocate(size_t NumObjects = 1) {
    assert(!isBorrowed);
    llvm::swift_demangle_detail::work();
    size_t ObjectSize = llvm::swift_demangle_detail::checkedSize(NumObjects, sizeof(T));
    if constexpr (std::is_same_v<T, Node>)
      llvm::swift_demangle_detail::parserNode();
    ...
    if (!CurPtr || reinterpret_cast<uintptr_t>(CurPtr) > reinterpret_cast<uintptr_t>(End) ||
        ObjectSize > reinterpret_cast<uintptr_t>(End) - reinterpret_cast<uintptr_t>(CurPtr)) {
      // No. We have to malloc a new slab.
      // We double the slab size for each allocated slab.
      if (ObjectSize > std::numeric_limits<size_t>::max() - alignof(T))
        throw llvm::swift_demangle_detail::LimitError{"Swift demangler allocation size overflow"};
      SlabSize = std::max(llvm::swift_demangle_detail::checkedSize(SlabSize, 2), ObjectSize + alignof(T));
      if (SlabSize > std::numeric_limits<size_t>::max() - sizeof(AllocatedSlab))
        throw llvm::swift_demangle_detail::LimitError{"Swift demangler allocation size overflow"};
      size_t AllocSize = sizeof(AllocatedSlab) + SlabSize;
      llvm::swift_demangle_detail::memory(AllocSize);
  ...
  template<typename T> void Reallocate(T *&Objects, uint32_t &Capacity,
                                       size_t MinGrowth) {
    assert(!isBorrowed);
    llvm::swift_demangle_detail::work(MinGrowth);
    if (MinGrowth > UINT32_MAX - Capacity)
      throw llvm::swift_demangle_detail::LimitError{"Swift demangler container size overflow"};
    size_t OldAllocSize = llvm::swift_demangle_detail::checkedSize(Capacity, sizeof(T));
    size_t AdditionalAlloc = llvm::swift_demangle_detail::checkedSize(MinGrowth, sizeof(T));
    ...
    if ((char *)Objects + OldAllocSize == CurPtr
        && AdditionalAlloc <= static_cast<size_t>(End - CurPtr)) {
    ...
    if (Growth < size_t(Capacity) * 2)
      Growth = size_t(Capacity) * 2;
    if (Growth > UINT32_MAX - Capacity)
      throw llvm::swift_demangle_detail::LimitError{"Swift demangler container size overflow"};
    llvm::swift_demangle_detail::work(Capacity);
...
protected:
  ...
  bool nextIf(StringRef str) {
    llvm::swift_demangle_detail::work(str.size());
