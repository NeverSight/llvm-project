...
#include "SwiftDemangleBudget.h" // LLVM integration: bounded allocations.
...
bool Punycode::decodePunycode(StringRef InputPunycode,
                              llvm::swift_demangle_detail::Vector<uint32_t> &OutCodePoints) {
  OutCodePoints.clear();
  ...
  while (!InputPunycode.empty()) {
    ...
    llvm::swift_demangle_detail::work(OutCodePoints.size() + 1);
...
bool Punycode::encodePunycode(const llvm::swift_demangle_detail::Vector<uint32_t> &InputCodePoints,
  ...
  for (auto C : InputCodePoints) {
    if (C < 0x80) {
      ++h;
      llvm::swift_demangle_detail::appendChar(OutPunycode, C);
    }
  ...
  if (b > 0)
    llvm::swift_demangle_detail::appendChar(OutPunycode, delimiter);
    ...
    for (auto c : InputCodePoints) {
      ...
      if (c == n) {
        ...
        for (int k = base; ; k += base) {
          ...
          if (q < t) break;
          llvm::swift_demangle_detail::appendChar(OutPunycode, digit_value(t + ((q - t) % (base - t))));
          q = (q - t) / (base - t);
        }
        llvm::swift_demangle_detail::appendChar(OutPunycode, digit_value(q));
...
static bool encodeToUTF8(const llvm::swift_demangle_detail::Vector<uint32_t> &Scalars,
                         std::string &OutUTF8) {
  for (auto S : Scalars) {
    ...
    switch (Bytes) {
    case 1:
      llvm::swift_demangle_detail::appendChar(OutUTF8, S);
      break;
    case 2: {
      uint8_t Byte2 = (S | 0x80) & 0xBF;
      S >>= 6;
      uint8_t Byte1 = S | 0xC0;
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte1);
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte2);
      break;
    }
    case 3: {
      ...
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte1);
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte2);
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte3);
      break;
    }
    case 4: {
      ...
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte2);
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte3);
      llvm::swift_demangle_detail::appendChar(OutUTF8, Byte4);
      break;
    }
    }
  }
...
static bool convertUTF8toUTF32(llvm::StringRef InputUTF8,
                               llvm::swift_demangle_detail::Vector<uint32_t> &OutUTF32,
...
bool Punycode::encodePunycodeUTF8(StringRef InputUTF8,
                                  std::string &OutPunycode,
                                  bool mapNonSymbolChars) {
  llvm::swift_demangle_detail::Vector<uint32_t> InputCodePoints;
