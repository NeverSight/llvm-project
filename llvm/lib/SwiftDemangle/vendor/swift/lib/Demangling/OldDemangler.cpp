...
public:
  ...
  bool readUntil(char c, std::string &result) {
    ...
    while (!isEmpty() && (c2 = peek()).value() != c) {
      llvm::swift_demangle_detail::appendChar(result, c2.value());
      advanceOffset(1);
    }
...
class OldDemangler {
  llvm::swift_demangle_detail::Vector<NodePointer> Substitutions;
  NameSource Mangled;
  NodeFactory &Factory;
...
#define DEMANGLE_CHILD_AS_NODE_OR_RETURN(PARENT, CHILD_KIND, DEPTH)            \
  ...
  NodePointer demangleTypeName(unsigned depth) {
    llvm::swift_demangle_detail::ParserScope BudgetScope; return demangleType(depth); }
...
#undef FUNCSIGSPEC_CREATE_PARAM_PAYLOAD
  ...
  demangleIdentifier(unsigned depth,
    ...
    if (isOperator) {
      ...
      llvm::swift_demangle_detail::reserveString(opDecodeBuffer, identifier.size());
      for (signed char c : identifier) {
        if (c < 0) {
          // Pass through Unicode characters.
          llvm::swift_demangle_detail::appendChar(opDecodeBuffer, c);
          continue;
        }
        if (c < 'a' || c > 'z')
          return nullptr;
        ...
        if (o == ' ')
          return nullptr;
        llvm::swift_demangle_detail::appendChar(opDecodeBuffer, o);
      }
