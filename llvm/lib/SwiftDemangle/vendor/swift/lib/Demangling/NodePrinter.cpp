...
DemanglerPrinter &DemanglerPrinter::operator<<(unsigned long long n) & {
  ...
  llvm::swift_demangle_detail::appendString(Stream, buffer);
  return *this;
}
...
matchSequenceOfKinds(NodePointer start,
                     llvm::swift_demangle_detail::Vector<std::tuple<Node::Kind, size_t>> pattern) {
  if (start != nullptr) {
  ...
  if (root->getNumChildren() >= 1) {
    ...
    if (firstChild->getKind() == Node::Kind::KeyPathGetterThunkHelper) {
      ...
      case Node::Kind::Subscript: {
        ...
        llvm::swift_demangle_detail::Vector<std::string> argumentTypeNames;
